...
pub const TXT_N_LAYERS: usize = 1; //24;
...
impl Module<(GraphTensor, KVCache)> for TextBlock {
    ...
    fn forward(&self, (x, cache): (GraphTensor, KVCache)) -> Self::Output {
        x.diff("./bins/_prefill_prompt_1.bin", 1e-5);
        x.diff("./bins/layer_norm1.bin", 1e-5);
        //confirm input, weights, and response
...
impl Module<(GraphTensor, GraphTensor, &[KVCache])> for Moondream {
    ...
    fn forward(&self, (img, toks, cache): (GraphTensor, GraphTensor, &[KVCache])) -> Self::Output {
        ...
        toks.diff("./bins/generate_text1.bin", 1e-5);

        for tok in 0..self.lm_head.weight.data().len() {
            println!("{:?}", toks.data().get(tok))
        }

        // Text embedding
        let mut x: GraphTensor = self.embed.forward(toks); // (b,seq,TXT_DIM)
        x.diff("./bins/_prefill_prompt_1.bin", 1e-5);
        ...
        // x.diff("./bins/decoder.bin", 1e-5);
        let res = self.lm_head.forward(self.txt_norm.forward(x));
        // println!("{:?}", res.shape);
        (res, new)
    }
}
