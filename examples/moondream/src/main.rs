...
pub struct CLIArgs {
    ...
    #[clap(short = 'p', long = "prompt", default_value = include_str!("../prompts/hi.txt"))]
    prompt: String,
}

fn main() {
    print!("Defining graph");
    let mut txt_cx = Graph::new(); // main graph for text
    let mut img_cx = Graph::new(); // main graph for img
    let cli_args = CLIArgs::parse();
    let tokenizer = Tokenizer::from_file("setup/tokenizer.json").unwrap();

    let mut input = txt_cx.named_tensor("Input", (1, 's'));
    ...
    let img = img_cx.named_tensor("Image", (10, 3, 378, 378));
    loader::load_image_binary_with_path(&img, &mut img_cx, "./prompts/image.bin");

    // (1) prefill with image
    let model = image_model::VisionModel::new(&mut img_cx);
    let mut model_weights = params(&model);
    loader::load("setup/moondream2.safetensors", &model, &mut img_cx);
    let img = img_cx.named_tensor("Image", (10, 3, 378, 378));
    loader::load_image_binary_with_path(&img, &mut img_cx, "./prompts/image.bin");

    let mut img_emb = model.forward(img);
    img_emb.diff("./bins/img_emb.bin", ATOL, RTOL);
    ...
    // );
    // let max = 730 + 1;
    // let (mut logits, mut cache_dst) = model.forward((input, &kv_cache, 730, max));
