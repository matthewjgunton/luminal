...
mod general_swap_tests {
    ...
    #[test]
    pub fn general_loop_swap() {
        let expr = r#"
            (let full (LoopOut (LoopOut (GMEM "x") (Loop "inner" (MNum 5)) (MVar "innerStride")) (Loop "outer" (MNum 10)) (MVar "outerStride")))
        "#;
        let report = run_egglog_test(expr);
        assert_rule_hits_at_least(
            &report,
            "(rewrite (LoopOut (LoopOut ?x ?innerLoop ?innerStride) ?outerLoop ?outerStride) (LoopOut (LoopOut (SwapLoops ?x ?innerLoop ?outerLoop) ?outerLoop ?outerStride) ?innerLoop ?innerStride) :ruleset ir)",
            1,
        );
    }
